...
import { getVideoCurriculum } from "@/lib/video-curriculum";
import { getCodeProject } from "@/lib/code-projects";
import { buildLessonMission } from "@/lib/lesson-missions";
import { expandStudyMaterials } from "@/lib/study-materials";
...
function normalizeLesson(fileName) {
  ...
  const video = getVideoCurriculum(slug);
  const baseLesson = {
    ...data,
    slug,
    video,
  ...
  const mission = buildLessonMission(baseLesson);
  const studyMaterials = expandStudyMaterials(baseLesson);
  ...
  return {
    ...
    mission,
    project,
    projectCheckCount: project?.tests.length || mission.steps.length,
  };
}
...
export function getTracks() {
  ...
  return Array.from(grouped.entries()).map(([slug, lessons]) => ({
    ...
    ...(TRACK_META[slug] || {
      ...
      description: "A Binary Tree learning track.",
...
export function getAdjacentLessons(slug) {
  ...
  const lessons = getAllLessons().filter((lesson) => lesson.trackSlug === current.trackSlug);
